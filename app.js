...
function getSearchText(mat) {
  const period = periodIsCond(mat.periodo) ? PERIODO_COND : `${mat.periodo}º Período`;
  return normalizeStr(`${formatName(mat)} ${mat.codigo} ${period}`);
}
...
function filterMainSearch(query) {
  ...
  const searchTerm = aliasesPesquisaDisciplinas && aliasesPesquisaDisciplinas[normalized]
    ? normalizeStr(aliasesPesquisaDisciplinas[normalized])
    : normalized;
  ...
  const matches = disciplinas.filter(mat => getSearchText(mat).includes(searchTerm));
  const matchCodes = new Set(matches.map(mat => mat.codigo));
  ...
  results.textContent = `${matches.length} ${matches.length === 1 ? 'resultado' : 'resultados'}`;
  ...
  suggestions.innerHTML = matches.slice(0, 6).map(mat => `
    <button type="button" class="search-suggestion text-left" data-search-code="${escapeHTML(mat.codigo)}" role="option">
      <div class="search-suggestion-main">
        <div class="search-suggestion-name">${escapeHTML(formatName(mat))}</div>
